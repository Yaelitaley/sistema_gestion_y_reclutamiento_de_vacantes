<?php
session_start();
/* Codigo leylani backend para recuperar contraseñas , NO TOCAR */
...
// Verificar que el correo existe y es reclutador (rol_id = 3)
$stmt = $conn->prepare("SELECT id FROM usuarios WHERE email = ? AND rol_id = 3");
...
$enlace = "http://localhost/sistema_gestion_y_reclutamiento_de_vacantes/reclutador/reset_password.php?token=" . $token;
...
try {
    ...
    $mail->setFrom('noreply@occ-empleo.com', 'OCC Empleo');
    ...
    $mail->Body    = '
        <div style="font-family: Poppins, sans-serif; max-width: 500px; margin: auto; padding: 30px; border-radius: 10px; border: 1px solid #ddd;">
            <h2 style="color: #fd7e14;">Recuperación de contraseña</h2>
            <p>Haz clic en el siguiente botón para restablecer tu contraseña. El enlace expira en <strong>1 hora</strong>.</p>
            <div style="text-align: center; margin: 30px 0;">
                <a href="' . $enlace . '"
                   style="background-color: #fd7e14; color: white; padding: 12px 30px; border-radius: 6px; text-decoration: none; font-weight: bold;">
